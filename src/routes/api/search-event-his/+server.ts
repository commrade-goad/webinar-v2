...
export const POST: RequestHandler = async ({ request, cookies }) => {
  try {
    ...
    if (b.status) params.append('status', b.status);
    if (b.type) params.append('type', b.type);
    if (b.role) params.append('role', b.role);
    
    const url = `${env.PRIVATE_API_URL}/api/protected/event-participate-of-user-ws?${params.toString()}`;
