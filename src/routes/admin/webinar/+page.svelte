<script lang="ts">
	...
	function openEditForm(webinar: IEvent) {
		...
		const start = new Date(webinar.EventDStart);
		const end = new Date(webinar.EventDEnd);

		// Format dates and times for form inputs
		eventDstart = `${start.getFullYear()}-${String(start.getMonth() + 1).padStart(2, '0')}-${String(start.getDate()).padStart(2, '0')}`;
		eventTimeStart = `${String(start.getHours()).padStart(2, '0')}:${String(start.getMinutes()).padStart(2, '0')}`;

		eventDend = `${end.getFullYear()}-${String(end.getMonth() + 1).padStart(2, '0')}-${String(end.getDate()).padStart(2, '0')}`;
		eventTimeEnd = `${String(end.getHours()).padStart(2, '0')}:${String(end.getMinutes()).padStart(2, '0')}`;
	...
	async function fetchWebinars() {
		try {
			...
			webinars.sort((a, b) => new Date(b.EventDStart).getTime() - new Date(a.EventDStart).getTime());
		} catch (err) {
	...
	const handleSubmit = async (event: Event) => {
		...
		const startDateTime = new Date(`${eventDstart}T${eventTimeStart}:00`);
		const endDateTime = new Date(`${eventDend}T${eventTimeEnd}:00`);
		
		// Create webinar object
		const webinarData: Partial<IEvent> = {
			EventName: eventName,
			EventDesc: eventDesc,
			EventSpeaker: eventSpeaker,
			EventDStart: startDateTime.toISOString(),
			EventDEnd: endDateTime.toISOString(),
			EventLink: eventLink,
		    EventMax: eventMax,
			EventAtt: eventAtt,
			EventImg: eventImg
		...
		if (isEditing && currentWebinarId !== null) {
			webinarData.ID = currentWebinarId;
		}
...
<Body>
	<div class="flex justify-between items-center mb-6">
		...
		<button 
			class="px-4 py-2 bg-sky-600 text-white rounded-lg hover:bg-sky-700 transition-colors flex items-center gap-2"
			onclick={openAddForm}
		>
	...
	{#if isFormOpen}
		...
		>
			<form onsubmit={handleSubmit} class="space-y-4">
				<div>
					<p class="block text-sm font-medium text-gray-700 mb-1">Event Name</p>
					<input 
						type="text" 
				...
				<div>
					<p class="block text-sm font-medium text-gray-700 mb-1">Speaker</p>
					<input 
						type="text" 
				...
				<div class="grid grid-cols-1 md:grid-cols-2 gap-4">
					<div>
						<p class="block text-sm font-medium text-gray-700 mb-1">Start Date</p>
						<input 
							type="date" 
					...
					<div>
						<p class="block text-sm font-medium text-gray-700 mb-1">Start Time</p>
						<input 
							type="time" 
				...
				<div class="grid grid-cols-1 md:grid-cols-2 gap-4">
					<div>
						<p class="block text-sm font-medium text-gray-700 mb-1">End Date</p>
						<input 
							type="date" 
					...
					<div>
						<p class="block text-sm font-medium text-gray-700 mb-1">End Time</p>
						<input 
							type="time" 
				...
				<div>
					<p class="block text-sm font-medium text-gray-700 mb-1">Event Link</p>
					<input 
						type="text" 
				...
				<div class="grid grid-cols-1 md:grid-cols-2 gap-4">
					<div>
						<p class="block text-sm font-medium text-gray-700 mb-1">Maximum Participants</p>
						<input 
							type="number" 
							bind:value={eventMax}
							min="1"
					...
					<div>
						<p class="block text-sm font-medium text-gray-700 mb-1">Event Type</p>
						<select
							bind:value={eventAtt}
							class="w-full px-3 py-2 border border-gray-300 rounded-md shadow-sm focus:outline-none focus:ring-sky-500 focus:border-sky-500"
							required
				...
				<div>
					<p class="block text-sm font-medium text-gray-700 mb-1">Image URL</p>
					<input 
						type="text" 
						bind:value={eventImg}
				...
				<div class="flex justify-end gap-3 pt-2">
					<button 
						type="button"
						onclick={closeForm}
						class="px-4 py-2 border border-gray-300 rounded-md shadow-sm text-sm font-medium text-gray-700 bg-white hover:bg-gray-50 focus:outline-none focus:ring-2 focus:ring-sky-500"
					>
						Cancel
					</button>
					<button 
						type="submit"
						class="px-4 py-2 border border-transparent rounded-md shadow-sm text-sm font-medium text-white bg-sky-600 hover:bg-sky-700 focus:outline-none focus:ring-2 focus:ring-sky-500"
					>
					...
					<div class="grid grid-cols-1 md:grid-cols-3 gap-4">
						<div class="col-span-2">
							<div class="flex items-start space-x-4">
								...
								{/if}
								<div>
									<p class="text-sm mb-2"><strong>Start:</strong> {formatDate(webinar.EventDStart)}</p>
									<p class="text-sm mb-2"><strong>End:</strong> {formatDate(webinar.EventDEnd)}</p>
									<p class="text-sm mb-2">
										<strong>Type:</strong> {webinar.EventAtt === 'online' ? 'Online' : 'Offline'}
									</p>
									<p class="text-sm mb-2">
										<strong>Max Participants:</strong> {webinar.EventMax}
									</p>
									{#if webinar.EventLink}
										<p class="text-sm mb-2">
											<strong>Link:</strong> 
											<a href={webinar.EventLink} target="_blank" class="text-sky-600 hover:underline">
												Join Webinar
											</a>
										</p>
									{/if}
							...
							{#if webinar.EventDesc}
								<p class="text-sm text-gray-600 mt-2">{webinar.EventDesc}</p>
							{/if}
							
							{#if webinar.EventParticipants && webinar.EventParticipants.length > 0}
								<p class="text-sm mt-2">
									<strong>Current Participants:</strong> {webinar.EventParticipants.length} / {webinar.EventMax}
								</p>
							{/if}
						</div>
						
						<div class="flex flex-col md:items-end justify-between gap-2">
							<div class="flex gap-2">
								<button 
									onclick={() => openEditForm(webinar)}
									class="px-3 py-1.5 bg-amber-500 text-white rounded hover:bg-amber-600 transition-colors flex items-center gap-1"
								>
								...
								<button 
									...
									class="px-3 py-1.5 bg-red-500 text-white rounded hover:bg-red-600 transition-colors flex items-center gap-1"
								>
									<svg xmlns="http://www.w3.org/2000/svg" class="h-4 w-4" viewBox="0 0 20 20" fill="currentColor">
							...
							<span class={`px-2.5 py-0.5 rounded-full text-xs font-medium ${
								new Date(webinar.EventDStart) <= new Date() && new Date(webinar.EventDEnd) >= new Date()
